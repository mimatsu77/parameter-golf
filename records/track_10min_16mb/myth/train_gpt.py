"""
RecurrentGPT for OpenAI Parameter Golf Challenge.
Core idea: 5 physical transformer layers x 4 recurrence passes = 20 effective layers.
Features: SwiGLU FFN, BigramHash embedding, sinusoidal PE, mixed Int5/Int6/Int8 quantization.
"""
...
try:
    ...
    _compress = lambda data: _zstd.ZstdCompressor(level=22).compress(data)
    _decompress = lambda data: _zstd.ZstdDecompressor().decompress(data)
    _COMPRESS_TAG = "zstd-22"
except ImportError:
    _compress = lambda data: zlib.compress(data, level=9)
...
# MUON OPTIMIZER
# ============================================================================
# Borrowed from modded-nanogpt. Background: https://kellerjordan.github.io/posts/muon/
...
# ============================================================================
# BPB (bits-per-byte) calculation for tokenizer-independent scoring.
...
# ============================================================================
# MIXED QUANTIZATION: FP16 / Int6 / Int5 / Int8
# ============================================================================
# Tiered quantization to maximize compression while preserving precision
# where it matters most. All int variants stored as int8 with restricted ranges;
# the restricted value set compresses well under zstd.

CONTROL_TENSOR_NAME_PATTERNS = tuple(
    p for p in os.environ.get(
        "CONTROL_TENSOR_NAME_PATTERNS",
        "attn_scale,mlp_scale,resid_mix,q_gain,mix_alpha,bigram.scale",
    ).split(",") if p
)
...
def quantize_int5_per_row(t: Tensor) -> tuple[Tensor, Tensor]:
    """Per-row int5 quantization: q in [-16, 15], stored as int8."""
    t32 = t.float()
    if t32.ndim == 2:
        row_max = t32.abs().amax(dim=1)
        scale = (row_max / 15.0).clamp_min(1e-12).to(INT8_PER_ROW_SCALE_DTYPE)
        q = torch.clamp(torch.round(t32 / scale.float()[:, None]), -16, 15)
        return q.to(torch.int8).contiguous(), scale.contiguous()
    amax = t32.abs().max().item()
    scale = torch.tensor(max(amax / 15.0, 1e-12), dtype=INT8_PER_ROW_SCALE_DTYPE)
    q = torch.clamp(torch.round(t32 / scale.float()), -16, 15)
    return q.to(torch.int8).contiguous(), scale.contiguous()


def quantize_int6_per_row(t: Tensor) -> tuple[Tensor, Tensor]:
    """Per-row int6 quantization: q in [-32, 31], stored as int8."""
    t32 = t.float()
    if t32.ndim == 2:
        row_max = t32.abs().amax(dim=1)
        scale = (row_max / 31.0).clamp_min(1e-12).to(INT8_PER_ROW_SCALE_DTYPE)
        q = torch.clamp(torch.round(t32 / scale.float()[:, None]), -32, 31)
        return q.to(torch.int8).contiguous(), scale.contiguous()
    amax = t32.abs().max().item()
    scale = torch.tensor(max(amax / 31.0, 1e-12), dtype=INT8_PER_ROW_SCALE_DTYPE)
    q = torch.clamp(torch.round(t32 / scale.float()), -32, 31)
    return q.to(torch.int8).contiguous(), scale.contiguous()


def quantize_float_tensor(t: Tensor) -> tuple[Tensor, Tensor]:
    """Per-row int8 quantization with percentile clipping."""
    t32 = t.float()
    if t32.ndim == 2:
        clip_abs = (
...
def _classify_param(name: str) -> str:
    """Classify parameter for mixed-precision quantization dispatch."""
    if "tok_emb" in name:
        return "embed"
    if ".attn.c_q." in name or ".attn.c_k." in name:
        return "attn_qk"
    if ".ffn." in name:
        return "ffn"
    return "other"
...
def quantize_state_dict_mixed(
    ...
    """Mixed-precision quantization: FP16 embeds, Int6 attn Q/K, Int5 FFN, Int8 rest."""
    ...
    for name, tensor in state_dict.items():
        ...
        # Small tensors: passthrough as FP16/FP32
        ...
        # Embeddings preserved in FP16 for maximum quality
        if cat == "embed":
...
def dequantize_state_dict(obj: dict[str, object]) -> dict[str, Tensor]:
    """Dequantize mixed-precision state dict. Format-compatible with int8-only too."""
    out: dict[str, Tensor] = {}
    qmeta = obj.get("qmeta", {})
...
# ============================================================================
# MODEL: RecurrentGPT with Enriched Input
# ============================================================================
...
class CastedLinear(nn.Linear):
    """Keep weights in fp32 for optimizer quality, cast at matmul time for bf16."""
    def forward(self, x: Tensor) -> Tensor:
...
class BigramHashEmbedding(nn.Module):
    ...
    def bigram_hash(self, tokens: Tensor) -> Tensor:
        ...
        out[..., 0] = mod  # first position: no previous token
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Manual GQA expansion for broad GPU compatibility
        if self.num_kv_heads != self.num_heads:
            reps = self.num_heads // self.num_kv_heads
            k = k.unsqueeze(2).expand(-1, -1, reps, -1, -1).reshape(
                bsz, self.num_heads, seqlen, self.head_dim
            )
            v = v.unsqueeze(2).expand(-1, -1, reps, -1, -1).reshape(
                bsz, self.num_heads, seqlen, self.head_dim
            )
...
class RecurrentBlock(nn.Module):
    """Single transformer block, reused across recurrence passes."""
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int,
    ...
    def forward(self, x: Tensor, x0: Tensor) -> Tensor:
        ...
        x = x + self.mlp_scale.to(dtype=x.dtype)[None, None, :] * self.ffn(self.mlp_norm(x))
        return x


class RecurrentGPT(nn.Module):
    """
    Depth-recurrent GPT: physical layers are reused across multiple passes.
    Enriched input = token embedding + sinusoidal PE + bigram hash embedding.
    5 physical layers x 4 recurrence = 20 effective layers.
    """
    def __init__(
        self,
        vocab_size: int,
        ...
        # -- Enriched Input Layer --
        self.tok_emb = nn.Embedding(vocab_size, model_dim)
        self.bigram = BigramHashEmbedding(bigram_hash_size, bigram_proj_dim, model_dim)
        # Sinusoidal position encoding (RoPE-frequency-based, parameter-free)
        pe = self._build_sinusoidal_pe(max_seq_len, model_dim, rope_base)
        ...
        # Learnable residual mixing coefficient per recurrence pass
        # Initialized at sigmoid(-3) ~ 0.047, so x flows through mostly unchanged
        self.mix_alpha = nn.Parameter(
            torch.full((recurrence_factor, model_dim), -3.0, dtype=torch.float32)
        )
    ...
    @staticmethod
    def _build_sinusoidal_pe(max_seq_len: int, dim: int,
                              base: float = 10000.0) -> Tensor:
        """Build sinusoidal PE using RoPE frequency schedule. Zero parameters."""
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # Enriched input: token emb + sinusoidal PE + bigram hash (3 components added)
        x = self.tok_emb(input_ids)
        x = x + self.pos_encoding[:, :seq_len, :].to(dtype=x.dtype)
        x = x + self.bigram(input_ids)
        x = F.rms_norm(x, (x.size(-1),))
        x0 = x

        # Depth recurrence: iterate physical layers multiple times
        for r in range(self.recurrence_factor):
            for block in self.blocks:
                x = block(x, x0)
            # Residual mixing: blend processed x with original x0
            alpha = torch.sigmoid(self.mix_alpha[r]).to(dtype=x.dtype)
            x = (1.0 - alpha[None, None, :]) * x + alpha[None, None, :] * x0
...
def main() -> None:
    ...
    if not args.tokenizer_path.endswith(".model"):
        raise ValueError(f"Only SentencePiece .model supported: {args.tokenizer_path}")
    ...
    base_model = RecurrentGPT(
        ...
        bigram_proj_dim=args.bigram_proj_dim,
    ).to(device).bfloat16()
    ...
    # Optimizer split:
    # - Muon for 2D matrix params in blocks + bigram projection
    # - Adam for embeddings, scalars, bigram table, recurrence mix_alpha
    ...
    # Bigram projection is a proper weight matrix -> Muon
    if base_model.bigram.proj is not None:
        matrix_params.append(base_model.bigram.proj.weight)
    # Recurrence mixing + bigram scale -> Adam scalars
    scalar_params.append(base_model.mix_alpha)
    scalar_params.append(base_model.bigram.scale)

    token_lr = args.tied_embed_lr if args.tie_embeddings else args.embed_lr
    # Token embedding + bigram embedding table share Adam with embed_lr
    embed_params_list = [base_model.tok_emb.weight, base_model.bigram.embed.weight]
    ...
    if base_model.lm_head is not None:
        optimizer_head = torch.optim.Adam(
            [{"params": [base_model.lm_head.weight], "lr": args.head_lr,
    ...
    log0(
        f"architecture:recurrent num_layers:{args.num_layers} "
        f"recurrence:{args.recurrence_factor} "
        f"effective_depth:{args.num_layers * args.recurrence_factor}"
    )
    ...
    log0(
        ...
        f"bigram_proj_dim:{args.bigram_proj_dim} compression:{_COMPRESS_TAG}"
    )
    ...
    # Warmup primes compiled forward/backward, then restores initial state
    ...
    while True:
        ...
        muon_momentum = (
            (1 - frac) * args.muon_momentum_warmup_start + frac * args.muon_momentum
        )
        for group in optimizer_muon.param_groups:
        ...
        # Wallclock cap sync across ranks
        reached_cap = (
    ...
    quant_file = "final_model.mixed.ptz"
    ...
    if master_process:
        ...
        ratio = quant_stats["baseline_tensor_bytes"] / max(
            quant_stats["int_payload_bytes"], 1
        )
        log0(
            f"Serialized model mixed+{_COMPRESS_TAG}: {quant_file_bytes} bytes "
            f"(payload:{quant_stats['int_payload_bytes']} raw_torch:{quant_raw_bytes} "
            f"payload_ratio:{ratio:.2f}x)"
        )
        log0(
            f"Total submission size mixed+{_COMPRESS_TAG}: "
    ...
    quant_state = torch.load(
        io.BytesIO(_decompress(quant_blob_disk)), map_location="cpu"
    )
    ...
    torch.cuda.synchronize()
    log0(
        f"final_mixed_{_COMPRESS_TAG}_roundtrip val_loss:{q_val_loss:.4f} "
        f"val_bpb:{q_val_bpb:.4f} "
        f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
    )
    log0(
        f"final_mixed_{_COMPRESS_TAG}_roundtrip_exact "
